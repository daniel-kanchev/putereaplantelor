...
class DatabasePipeline:
    # Database setup
    conn = sqlite3.connect('putereaplantenor.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute(""" CREATE TABLE IF NOT EXISTS articles
        (title text, date text, author text, categories text, tags text, link text, content text) """)
    ...
    def process_item(self, item, spider):
        ...
        # Insert values
        self.c.execute("INSERT INTO articles (title, date, author, categories, tags, link, content)"
